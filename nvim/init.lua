...
require('lazy').setup({
  ...
  {
    -- You can easily change to a different colorscheme by uncommenting the desired one.
    -- Available themes (uncomment to use):
    ...
    init = function()
      ...
      -- Optional: Configure custom highlights
...
vim.api.nvim_create_autocmd('BufEnter', {
  pattern = '*',
  callback = function()
    vim.fn.matchadd('Conceal', '!=', 100, -1, { conceal = '≠' })
    vim.fn.matchadd('Conceal', '<=', 100, -1, { conceal = '≤' })
    vim.fn.matchadd('Conceal', '>=', 100, -1, { conceal = '≥' })

    -- Set the highlight group for the Conceal text
    vim.cmd 'highlight! link Conceal Operator' -- Change 'Keyword' to any desired group
  end,
})
...
vim.diagnostic.config {
  virtual_text = diagnostics_enabled, -- Disables inline error messages
  signs = diagnostics_enabled, -- Keeps the error/warning signs in the sign column
  underline = diagnostics_enabled, -- Keeps the underline for errors
  update_in_insert = false, -- Disables updating diagnostics while typing
}
...
--
---- Evaluate text selected by an operator
...
--
---- Show or enter the output window
--vim.keymap.set('n', '<localleader>os', ':noautocmd MoltenEnterOutput<CR>', { silent = true, desc = 'Show/enter output' })
--
----vim.g.molten_automatic_open = 1
vim.cmd.colorscheme 'gruvbox'
vim.cmd 'hi Normal guibg=#1d2021' -- Change the background color
vim.cmd 'hi Normal guibg=#1' -- Change the background color
--vim.g.jukit_terminal = 'wezterm'
--
--
--
